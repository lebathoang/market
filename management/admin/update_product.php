        ...
        <div class="page-content col-lg-10 col-md-9" style="margin-left: 225px;">
            ...
            <div id="products" class="form tab-pane">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="dashboard-title">Cập nhập sản phẩm</h1>
                </div>
...
if (isset($_POST['save'])) {
    try {
        $sql = "UPDATE sanpham  SET `TenSP` = :TenSP, `Hinhanh` = :Hinhanh, `Gia` = :Gia, `Soluong` = :Soluong, `Mota` = :Mota, `LoaiSP` = :LoaiSP, `Danhmuc` = :Danhmuc, `Giamgia` = :Giamgia, `Xuatxu` = :Xuatxu WHERE `MaSP` = $id";
        $db = getDatabaseConnection();
        $statement = $db->prepare($sql);
